...
// MARK: - External Session Discovery

/// Pure-function utility for discovering Codex / Claude Code sessions on disk.
/// All methods are static and require no mutable state.
enum ExternalSessionDiscovery {
    ...
    static func discoverCodexSessions(limit: Int) -> [TerminalSession] {
        let root = UserPaths.homeDirectoryURL.appendingPathComponent(
            MainWindowController.ExternalSessionImport.codexRelativePath,
            isDirectory: true
        )
        let candidates = recentJSONLFiles(
            under: root,
            excludingPathComponents: [],
            maxCandidates: max(limit * MainWindowController.ExternalSessionImport.codexScanMultiplier, limit)
        )
    ...
    static func discoverClaudeSessions(limit: Int) -> [TerminalSession] {
        let root = UserPaths.homeDirectoryURL.appendingPathComponent(
            MainWindowController.ExternalSessionImport.claudeRelativePath,
            isDirectory: true
        )
        let candidates = recentJSONLFiles(
            under: root,
            excludingPathComponents: ["subagents", "tool-results"],
            maxCandidates: max(limit * MainWindowController.ExternalSessionImport.claudeScanMultiplier, limit)
        )
        guard !candidates.isEmpty else {
            return []
        }
    ...
    static func codexSession(from url: URL, fallbackDate: Date) -> TerminalSession? {
        let lines = prefixLines(from: url, maxBytes: MainWindowController.ExternalSessionImport.maxReadBytesPerFile, maxLines: 80)
    ...
    static func claudeSession(from url: URL, fallbackDate: Date) -> TerminalSession? {
        let lines = prefixLines(from: url, maxBytes: MainWindowController.ExternalSessionImport.maxReadBytesPerFile, maxLines: 120)
        guard !lines.isEmpty else {
            return nil
        }
